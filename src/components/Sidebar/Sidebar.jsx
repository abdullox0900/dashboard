    ...
    <>
      <nav className="w-[250px] h-lvh p-[30px]">
        ...
        <ul className="flex flex-col gap-[15px]">
          {DASHBOARD_SIDEBAR_LINKS.map((content, index) => <SidebarLink key={index} item={content} />)}
        </ul>
      </nav>
    </>
  );
...
function SidebarLink({item}) {
    return(
        ...
        <li
          className="flex gap-[5px] items-center py-[8px] px-[10px] text-[#67748e] font-medium hover:bg-[#fff] hover:text-[#252f40] rounded-[10px] active:bg-slate-300 duration-100 ease-in cursor-pointer"
        >
